...
function main() {
	...
	uploadForm.addEventListener('submit', async function (e){
		...
		console.log(e.target.clientUpload.files[0])
		console.table(file)
		let formData = new FormData()
		formData.append('file', file);
		console.table(formData[0])
...
async function bListGogleCal() {
	...
	if (data.events == "") screenList.innerHTML = `None`;
...
async function bInputTaskFile() {
	...
	const response = await prowrap(fetch("/api/settings/save", {
		method: "POST",
