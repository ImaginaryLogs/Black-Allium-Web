...
const eventCheckValid = (eventString) => {
	...
	if (
		dateParams["dateStart"]["dateIndex"] != -1 ||
		dateParams["dateEnd"]["dateIndex"] != -1
	) {
		validData = new taskEvents(
			summary,
			dateParams["dateStart"]["dateYMD"],
			dateParams["dateEnd"]["dateYMD"]
		);
	}
	return validData;
};
		...
		for (const publishedEvent of publishedEvents["events"]) {
			...
			for (let requestedEvent of requestedEvents["tasks"]) {
				...
				if (!publishedEvent["event"].localeCompare(requestedEvent.summary)) {
					isEventSync = true;
					syncedObj["tasks"].push(
						publishedEvent["event"].slice("OBS: ".length)
					);
